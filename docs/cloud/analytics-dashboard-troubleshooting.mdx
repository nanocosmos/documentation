---
slug: /analytics/dashboard/troubleshooting
id: analytics-dashboard-troubleshooting
...
The trouble shooting page helps to investigate stream or user specific metrics across the streaming experience from stream ingest to the playout at the end user. With this feature you have a complete transparent view at your streams, which helps you to find potential problems and be more capable of solving them faster.
...
- `User ID` - [user ID](../nanoplayer/nanoplayer_player_metrics#how-to-use).
- `IP` - client IP address used for playout.
- `Event ID` - [event ID](../nanoplayer/nanoplayer_player_metrics#how-to-use).
- `Custom field 1` - [custom field1](../nanoplayer/nanoplayer_player_metrics#how-to-use).
...
The Max Frame Delay metric helps you understand how quickly the ingest server processes your stream data.
It represents the maximum latency measured in milliseconds for frame buffering when a frame is received by the ingest server.

A higher max frame delay indicates higher latency for the playback/viewers which can result in buffering or freezes. As a rough reference, delays under 500 ms are generally considered good, delays between 500–1200 ms may indicate issues, and delays above 1200 ms are considered critical. For optimal and most reliable performance, aiming for a max frame delay below 200 ms is recommended.

Tracking this metric is important because it allows you to:
- `Identify potential issues early` – A high frame delay can signal problems on the ingest side, such as **limited bandwidth, high CPU load, or unstable network conditions**. Delay spikes highlight when these issues occur, allowing you to address them before they impact viewers.
- `Optimize your setup`– Tracking delay trends allows you to fine-tune streaming settings (e.g., bitrate, encoding, server choice) for the best possible quality.
- `Ensure smoother playback` – Lower delays provide real-time responsiveness with minimal buffering.

*Note* : The max frame delay is calculated by the ingest server based on the frequency of received frames. The offset is crucial for ensuring a smooth playback experience, minimizing buffering and preventing video/audio desynchronization.
<ScaledImage image="analytics-alert-max-time-frame-regular.png" alt="A Regular Max Frame Delay Example" widthScaling="50" title="A Regular Max Frame Delay Example" 
    description="Looking at this chart, most values are around 250–300 ms, with spikes reaching 350–375 ms, which we can consider a good range. Still, delays of 250–300 ms can cause minor issues, since it takes about a quarter of a second for the stream to reach viewers. For the smoothest experience, the goal is to keep the max frame delay under 200 ms. " />

<ScaledImage image="analytics-alert-max-time-frame-spike.png" alt="A Spiked Max Frame Delay Example" widthScaling="50" title="A Spiked Max Frame Delay Example" 
    description="Looking at this chart, most values hover around 400 ms, representing a 0.4-second delay that is generally within an acceptable range. However, a major spike reaching 1,500 ms around 09:30 indicates a severe delay that could cause a brief freeze or audio/video desync, with a high chance of creating a disruptive viewing experience at that moment." />
...
Via **Ingests/Alerts** tab you can examine alerted streams and make potential issues visible, identify their causes and get helpful recommendations to solve them. After entering a stream name you able to search for any alert occurrences in the past 4 weeks. In the following table you can select any alert to get the basic stream details right away.
...
(I) `Link to Documentation` - References more knowledge about alerts and how to handle them

<br/>

#### Alerts of Type - Inactive Ingest

When an **Inactive Ingest** alert is selected in the alert table, this widget displays information about the ingest connection that is connected but not actively sending data. This can indicate encoder configuration issues, network problems, or source-side failures.

![Screenshot: Alert Widget II](../assets/analytics/analytics-alert-widget-ii.png)
*Screenshot: Alert Widget II*
...
(C) `Activity Timeline Overview` - The activity timeline shows all open ingest connections and when they were active or inactive

(D) `Activity Timeline Legend` - The color descriptions for the displayed data (click to toggle visibility)

(E) `Example: Active Connection` - This example shows a regular ingest connection that is active 

(F) `Example: Inactive Connection` - This example shows an ingest connection that is inactive

(G) `Alert Details Panel` - Containing explanations and further details of this alert and also provides a recommended action to potentially solve the issue

(H) `Link to Documentation` - References more knowledge about alerts and how to handle them
...
(J) `Link to Documentation` - References more knowledge about alerts and how to handle them
...
(D) `Link to Documentation` - References more knowledge about alerts and how to handle them
