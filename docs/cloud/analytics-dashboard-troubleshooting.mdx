...
(C) `Activity Timeline Overview` - The activity timeline shows all open ingest connections and their when they were active or inactive

(D) `Activity Timeline Legend` - The color descriptions for the displayed data
...
(G) `Alert Details Panel` - Containing explanantions and further details of this alert and also provides a recommended action to potentially solve the issue
